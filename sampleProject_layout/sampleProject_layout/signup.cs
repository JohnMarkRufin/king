...
namespace sampleProject_layout
{
    public partial class signup : Form
    {
        ...
        {
            if (tbxFname.Text == "FIRSTNAME" || tbxLname.Text == "LASTNAME" || tbxContact.Text == "CONTACT NO" || tbxUsername.Text == "USERNAME" || tbxPassword.Text == "PASSWORD")
            {
                MessageBox.Show("PLEASE FILL ALL DATA", "Warning", MessageBoxButtons.OK,MessageBoxIcon.Warning);
            }
            else
            {
                main.signup(tbxFname.Text, tbxLname.Text, tbxContact.Text, tbxUsername.Text, tbxPassword.Text);
                MessageBox.Show("ACCOUNT SUCCESFULLY CREATED", "Message", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                new signin().Show();
                Visible = false;
            }
        }
